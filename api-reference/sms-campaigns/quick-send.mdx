...
<RequestExample>
```bash cURL
curl "https://sendazi.com/api/v1/v1/sms-campaigns/quick?to=233241234567&message=Hello%20from%20Sendazi!&sender_id=MYCOMPANY&api_key=YOUR_API_KEY"
```
...
fetch(`https://sendazi.com/api/v1/v1/sms-campaigns/quick?${params}`)
  .then(res => res.json())
  .then(console.log);
```
